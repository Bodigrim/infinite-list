...
main = defaultMain $ testGroup "All"
  ...
  , testProperty "lines" $
    \(Blind (xs :: Infinite Char)) ->
      I.take 3 (I.lines xs) == L.take 3 (L.lines (I.foldr1 (:) xs))
  ...
  , testProperty "words" $
    \(Blind (xs :: Infinite Char)) ->
      I.take 3 (I.map NE.toList (I.words xs)) == L.take 3 (L.words (I.foldr1 (:) xs))
  ...
  , testProperty "intersect" $
    \(Blind (xs :: Infinite Ordering)) ys -> not (null ys) ==>
      I.head (I.intersect xs ys) == L.head (I.foldr1 (:) xs `L.intersect` ys)
  ...
  , testProperty "!!" $
    \(Blind (xs :: Infinite Int)) n ->
      xs I.!! n == I.foldr1 (:) xs L.!! fromIntegral n
  ...
  , testProperty "elemIndex" $
    \xs (x :: Int) (Blind ys) ->
      let zs = I.prependList xs (x :< ys) in
        Just (fromIntegral (I.elemIndex x zs)) == L.elemIndex x (I.foldr1 (:) zs)
